...
## Fine-tuning
A more heavyweight approach is fine-tuning. Here, we literally retrain the neural network that embodies the knowledge embodied in the LLM. This is useful when you want the model to adopt a particular style, follow specific behavioral rules, or internalize patterns that are too complex to express in a prompt. The tradeoff is cost and rigidity: fine-tuning requires many examples, and can be quite costly. OpenAI has a fine-tuning API, and [it is fairly easy to use and build your own custom LLM](https://www.datacamp.com/tutorial/fine-tuning-openais-gpt-4-step-by-step-guide) using their interface, if you have training data. The main concern is cost. 

## RAG
The third approach sits between these two: retrieval-augmented generation, or RAG. Instead of trying to store new knowledge inside the model’s parameters, RAG retrieves relevant documents from an external source and injects those into the prompt at runtime. This can be as simple as keyword search (brittle) or can use  search over semantic embeddings in a database vector store. RAG systems are powerful because they combine an LLM’s reasoning with a searchable database, which can be updated at any time without retraining the model. If your business has large corpus of information that you need to quickly access to build a more accurate LLM responses, RAG is a great avenue. 

In this week's coming lessons, we will focus on RAG. We will start with the most brittle version, naive keyword-based retrieval, to highlight the basic logic of RAG. Then we move to semantic RAG, where chunks of text are converted to embeddings and stored in a vector database. This allows the system to retrieve information based on meaning rather than exact word matches. Finally, we will move to a framework (LlamaIndex) to recreate the same idea in a more production-ready form with much less code.
