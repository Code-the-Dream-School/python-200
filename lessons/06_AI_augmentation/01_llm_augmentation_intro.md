...
## RAG
The third approach sits between the above two in terms of complexity: retrieval-augmented generation, or RAG. Instead of trying to store new knowledge inside the model's parameters, RAG retrieves relevant informatoin from an external knowledge store (such as a database), and puts that information into the LLM prompt so that the LLM can have that information in the query. 

The way this knowledge retrieval works can be as simple as keyword search, or it can be more sophisticated semantic search over a  database vector store. RAG systems are powerful because they combine an LLM's reasoning with a searchable database: the database can be updated on premises at any time without retraining the model. 
...
### Where we are headed
In this week's coming lessons, partly because of their importance in industry, we will focus on RAG systems. We will start with the most brittle version, naive keyword-based retrieval, to highlight the basic logic of RAG. Then we move to semantic RAG, where chunks of text are converted to embeddings and stored in a vector database. Finally, we will move to a framework (LlamaIndex) to recreate the same idea in a more production-ready form with much less code. This is more like what you will see in industry. 
